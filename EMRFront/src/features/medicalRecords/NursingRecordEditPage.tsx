...
function getUpdateNursingRecordErrorMessage(error: unknown) {
  const status = getApiErrorStatus(error)

  if (status === 403) {
    return '현재 계정 권한으로는 간호기록을 수정할 수 없습니다.'
  }

  if (status === 404) {
    return '수정할 간호기록을 찾을 수 없습니다.'
  }

  return '간호기록 수정에 실패했습니다. 입력 정보를 확인해 주세요.'
}

export function NursingRecordEditPage() {
  ...
  const mutation = useMutation({
    mutationFn: (values: UpdateNursingRecordRequest) =>
      updateNursingRecord(parsedRecordId as number, values),
  ...
  const isNursingRecord = record?.type === 'NURSING'
  const canEditRecord =
    !!record &&
    isNursingRecord &&
    !!user &&
  ...
  return (
    <RoleGuard
      allowedRoles={['NURSE']}
      fallback={
        <ErrorMessage message="현재 계정 권한으로는 간호기록을 수정할 수 없습니다." />
      }
    >
      <div className="space-y-5">
        ...
        {recordQuery.error && (
          <ErrorMessage
            message={getUpdateNursingRecordErrorMessage(recordQuery.error)}
          />
        )}

        {mutation.error && (
          <ErrorMessage
            message={getUpdateNursingRecordErrorMessage(mutation.error)}
          />
        )}

        {record && !isNursingRecord && (
          <ErrorMessage message="간호기록만 이 화면에서 수정할 수 있습니다." />
        )}

        {record && isNursingRecord && !canEditRecord && (
          <ErrorMessage message="본인이 작성한 간호기록만 수정할 수 있습니다." />
        )}
