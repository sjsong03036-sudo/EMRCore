...
import { createAdmissionDischargeRecord } from '../../api/medicalRecordApi'
...
function getCreateAdmissionDischargeRecordErrorMessage(error: unknown) {
  const status = getApiErrorStatus(error)

  if (status === 403) {
    return '현재 계정 권한으로는 입퇴원기록을 작성할 수 없습니다.'
  }

  if (status === 404) {
    return '입퇴원기록을 작성할 환자를 찾을 수 없습니다.'
  }

  return '입퇴원기록 작성에 실패했습니다. 입력 정보를 확인해 주세요.'
}

export function AdmissionDischargeRecordCreatePage() {
  const navigate = useNavigate()
  const queryClient = useQueryClient()
  const [pendingValues, setPendingValues] =
    useState<CreateAdmissionDischargeRecordRequest | null>(null)

  const mutation = useMutation({
    mutationFn: createAdmissionDischargeRecord,
  ...
  const handleSubmit = (
    ...
    setPendingValues(values as CreateAdmissionDischargeRecordRequest)
  }
  ...
  return (
    ...
    >
      <div className="space-y-5">
        ...
        {mutation.error && (
          <ErrorMessage
            message={getCreateAdmissionDischargeRecordErrorMessage(
              mutation.error,
            )}
          />
        )}
        ...
        <ConfirmModal
          confirmLabel="저장"
          description="입력한 내용으로 입퇴원기록을 저장하시겠습니까?"
